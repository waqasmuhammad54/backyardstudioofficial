...
const nextConfig = {
  ...
  async redirects() {
    return [
      ...
      // Legacy generic slug shells that predate the service catalogue. No route file
      // ever existed for either; both resolve to the [slug] template today. Consolidate
      // to the same target the drone family already uses (see above) rather than
      // leaving indexable generic shells live.
      { source: "/services/drone",                           destination: "/services/real-estate",         permanent: true },
      { source: "/services/aerial",                          destination: "/services/real-estate",         permanent: true },
      // City-abbreviated slugs generated by older "Other Emirates" arrays
      // ("RAK"/"UAQ" → /locations/rak, /locations/uaq). The arrays are fixed in the
      // routes; these redirects close the loop for Google-held URLs.
      { source: "/locations/rak",                            destination: "/locations/ras-al-khaimah",     permanent: true },
      { source: "/locations/uaq",                            destination: "/locations/umm-al-quwain",     permanent: true },
      // Food-photography slug never had its own route; /services/food is the real
      // service page (Food Photography & Videography Dubai). Redirect preserves
      // any acquired links while the 8 internal hrefs are updated directly.
      { source: "/services/food-photography",                destination: "/services/food",               permanent: true },
